...
import { BlogPost, fetchBlogPosts, createBlogPost, updateBlogPost, deleteBlogPost } from "@/lib/firebase";
    ...
    <div className="space-y-6">
      ...
      {editing && (
        <div className="bg-white rounded-xl p-6 shadow-sm space-y-4">
          ...
          <Field label="Cover Image URL" value={editing.coverImage} onChange={(v) => updateEditing({ coverImage: v })} />
          <ImageUpload label="Cover Image" value={editing.coverImage} onChange={(v) => updateEditing({ coverImage: v })} folder="winit/blogs" />
          ...
          <div className="flex justify-end">
            <SaveButton onClick={save} saving={saving} disabled={!editing.title.trim()} label={editing.id && posts.find((p) => p.id === editing.id) ? "Update Post" : "Publish Post"} />
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <p className="text-gray-400 text-sm text-center py-12">No posts match your search or filter.</p>
      ) : (
